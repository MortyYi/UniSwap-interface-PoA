...
export default function useBuyNFTCallback(
            ...
            return {
                ...
                execute:
                    sufficientBalance
                        ? async () => {
                            try {
                                const txReceipt = await NFTMarketContract.buy(id, { value: mvalue })
                                addTransaction(txReceipt, { summary: `Buy NFT: ${id}  ` })
    ...
    }, [NFTMarketContract, chainId, balance, addTransaction, mvalue, id])
}